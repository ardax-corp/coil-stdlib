use num::{PI, E, TAU, abs, min, max, clamp, round, pow, signum, gcd, lcm, trunc, div_euclid, rem_euclid, hypot, is_finite};
...
test("PI E TAU") {
    ...
    assert(TAU < 6.29)?;
    assert(TAU == PI * 2.0)?;
    assert(abs(sin(PI / 2.0) - 1.0) < 0.000001)?;
    assert(abs(exp(1.0) - E) < 0.000001)?;
}
